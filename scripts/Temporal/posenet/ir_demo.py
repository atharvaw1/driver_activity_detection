...
def main():
    ...
    with tf.Session() as sess:
        ...
        h = 1
        # print('x')
        path = '/home/allgo/atharva/AllgoActivityIRData/'
        for root,dir,file in os.walk(path):
            ...
            if len(dir)==0:
                ...
                h += 1
                frame_list = []
                for i in sorted_alphanumeric(file):
                    if i[-4:] == '.jpg':
                        # print(root+'/'+i)
        ...
        # print(path_list)
            ...
            while frame_c<frame_n-1:
                ...
                # input_image= cv2.resize(input_image[0],(224,224))
                # # input_image = cv2.cvtColor(input_image,cv2.COLOR_BGR2GRAY)
                # # input_image = cv2.equalizeHist(input_image)
                # # input_image = cv2.cvtColor(input_image,cv2.COLOR_GRAY2RGB)
                # input_image = np.array([input_image])

                # draw_image= cv2.resize(draw_image,(224,224))
                # draw_image = cv2.cvtColor(draw_image,cv2.COLOR_BGR2GRAY)
                # draw_image = cv2.equalizeHist(draw_image)
